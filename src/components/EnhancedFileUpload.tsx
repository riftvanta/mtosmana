...
const EnhancedFileUpload: React.FC<EnhancedFileUploadProps> = ({
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    return new Promise((resolve, reject) => {
          ...
          try {
            ...
            if (response.success) {
              setUploadProgresses(prev => 
                prev.map(p => 
                  p.fileName === file.name 
                    ? { ...p, status: 'completed', url: response.url }
                    : p
                )
              );
              ...
              setUploadProgresses(prev => 
                prev.map(p => 
                  p.fileName === file.name 
                    ? { ...p, status: 'error', error: response.error }
                    : p
                )
              );
             ...
             setUploadProgresses(prev => 
               prev.map(p => 
                 p.fileName === file.name 
                   ? { ...p, status: 'error', error: 'Invalid response from server' }
                   : p
               )
             );
      ...
      const apiEndpoint = uploadMethod === 'firebase' ? '/api/upload' : '/api/upload-simple';
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      >
        ...
        <div className="text-xs text-gray-400">
          ...
          <p className="mt-1">
            Upload method: <span className="font-medium">{uploadMethod === 'firebase' ? 'Firebase Storage' : 'Local Storage (Dev)'}</span>
          </p>
        </div>
      </div>
